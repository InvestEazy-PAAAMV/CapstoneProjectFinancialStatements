...
def login():
    ...
    if request.method == 'POST':
        ...
        driver.find_element_by_xpath("//input[@id='search_str']").send_keys(user + "\n")
...
def get_speech():
    ...
    ds_keywords_final=[word for word in ds_keyword_tags.keys() if ds_keyword_tags[word]=='NN']
    cs_keywords_final=[word for word in cs_keyword_tags.keys() if cs_keyword_tags[word]=='NN']
    ...
    out=json.dumps([{'summary':cs_summ , 'keywords':cs_keywords_final[:5], 'fullCont':cs}, {'summary':ds_summ , 'keywords':ds_keywords_final[:5], 'fullCont':ds}]) 
    return out
...
def ratios():
	...
	ratio_list=list(['Basic EPS (Rs.)','Diluted EPS (Rs.)', 'Book Value [ExclRevalReserve]/Share (Rs.)', 'Dividend / Share(Rs.)','PBIT Margin (%)', 'Net Profit Margin (%)','Return on Networth / Equity (%)','Return on Capital Employed (%)','Return on Assets (%)', 'Total Debt/Equity (X)','Asset Turnover Ratio (%)', 'Current Ratio (X)','Quick Ratio (X)','Dividend Payout Ratio (NP) (%)','Earnings Yield'])
	ratio_final=ratio.loc[ratio_list]
	ratio_format=[]
	for ratio in ratio_list:
		values=list(ratio_final.loc[ratio])
		#print(values)
		ratio_format.append({'name':ratio, 'data':values, 'graph':'#chart1'})
...
def get_news_links():
	...
	driver.find_element_by_xpath("//input[@class='inputBox']").send_keys(user + "\n")
	...
	for link in news_links:
		#annual report
		driver.get(link)
		headline=driver.find_element_by_xpath('//h1[@class="clearfix title"]').text
